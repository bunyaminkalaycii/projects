...
// - Prune (budamak) unnecessary subtrees
...
void apply_move_to_tree(TreeNode **root, int move, int initial_tree_depth){
    TreeNode* oldNodePtr = *root;
    *root = oldNodePtr->children[move];
    for(int i = 0; i < oldNodePtr->num_children; i++){
        if(i != move)
            free_tree(oldNodePtr->children[i]);//KONTROL EDİLECEK
    }
    free_game_state(oldNodePtr->game_state);
    free(oldNodePtr->children);
    free(oldNodePtr);
    expand_tree(*root);
